...
test_that("brreg_entity type = label works", {
  skip_if_offline()
  eq <- brreg_entity("923609016", type = "label")
...
test_that("brreg_search returns results", {
  skip_if_offline()
  s <- brreg_search(name = "Equinor", max_results = 5)
...
test_that("brreg_search returns empty tibble for no matches", {
  skip_if_offline()
  s <- brreg_search(name = "ZZZZNOTEXIST999QQQQQ", max_results = 5)
...
test_that("brreg_updates returns results", {
  skip_if_offline()
  u <- brreg_updates(since = Sys.Date() - 2, size = 10)
...
test_that("brreg_updates include_changes works", {
  skip_if_offline()
  u <- brreg_updates(since = Sys.Date() - 2, size = 3, include_changes = TRUE)
