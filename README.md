...
Save dated bulk downloads as Hive-partitioned Parquet for panel
construction:
...
**Separate backends for separate scales.** `brreg_search()` uses the
JSON API (filtered, max 10,000). `brreg_download()` / `brreg_snapshot()`
use the bulk CSV (always full register). Both produce the same column
schema via `field_dict`.

**Snapshot-based temporal database.** `brreg_snapshot()` accumulates
Hive-partitioned Parquet files. `brreg_panel()` queries them lazily via
Arrow with partition pruning and projection pushdown.
